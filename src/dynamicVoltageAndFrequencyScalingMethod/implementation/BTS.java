...
/**
 * Blocking-Time Stealing implementation of Dynamic Voltage and Frequency Scaling Method.
 * @author YC
 * 
...
public class BTS extends DynamicVoltageAndFrequencyScalingMethod 
{
    ...
    /**
     * Inner class for high speed settings.
     */
        ...
        if(p.getController().getConcurrencyControlProtocol().getClass().getSimpleName().equals("SRP"))
        {
            ...
            switch(p.getSchedulingAlgorithm().getClass().getSimpleName())
            {
                case "RMS":
                    ...
                    baseSpeed = RTSimulatorMath.mul(this.S_b, p.getTaskSet().getProcessingSpeed());
                    
                    this.S_b = RTSimulatorMath.div(baseSpeed ,p.getCore(0).getParentCoreSet().getCoreSpeedSet().getMaxFrequencyOfSpeed());
