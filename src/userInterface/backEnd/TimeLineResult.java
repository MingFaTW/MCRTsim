...
/**
 * Timeline canvas panel that renders either core-centric or task-centric
 * scheduling results and supports interactive markers and attribute probing.
 *
 * <p>Behavior integrated from inline comments:</p>
 * <ul>
 *   <li>Preferred size depends on the mode: when {@link ScheduleResult#isCoreTimeLine}
 *       is true, height is based on the number of core timelines; otherwise it
 *       uses task timelines. Width is derived from (finalTime + 1) × base unit + margins.</li>
 *   <li>Tooltips for resource panels are made persistent by setting
 *       {@link ToolTipManager#setDismissDelay(int)} to {@code Integer.MAX_VALUE}.</li>
 *   <li>An HSB color palette ({@link #resourceColor}) is used to color resource
 *       legend and mini-bars; {@link #reverseColor(Color)} provides contrasting text.</li>
 *   <li>Mouse move updates the current time and highlights crosshair lines when the
 *       viewer is in {@link ViewerStatus#EXECUTION} mode; attribute details for the
 *       timeline under the cursor are propagated to the attributes panel.</li>
 *   <li>Mouse click (in EXECUTION mode) adds a {@link MouseTimeLine} marker at the
 *       current time, stores it in a combo box for selection, and repaints.</li>
 *   <li>Painting chooses between drawing core timelines or task timelines, then overlays
 *       interactive markers and the resource legend.</li>
 * </ul>
 *
 * <p>This panel uses absolute layout (null layout) so timeline children (resource panels
 * and mouse markers) can be positioned precisely in pixel coordinates.</p>
 *
 * @author ShiuJia
 */
public class TimeLineResult extends JPanel
{
    /** Parent schedule result that supplies data, scale and mode (core/task view). */
	public ScheduleResult parent;
    private int mouseX;
    private int mouseY;
    /** Current mouse interaction mode (IDLE vs. EXECUTION for crosshair/marker placement). */
    ...
    /**
     * Constructs the timeline canvas, initializes palette, sizing and listeners,
     * and pre-populates resource panels for each timeline according to the mode.
     *
     * @param sr the backing {@link ScheduleResult} (provides timelines and scale)
     */
    ...
    /**
     * Paint entry point: delegates to {@link #drawCoreTimeLine(Graphics)} or
     * {@link #drawTaskTimeLine(Graphics)} depending on the current mode.
     * Always calls {@code super.paintComponent(g)} first.
     */
    ...
    /**
     * Overrides {@link JPanel#paint(Graphics)} to delegate to
     * {@link #paintComponent(Graphics)} explicitly. This keeps a single drawing
     * code path for both Swing paint cycles and direct paint calls from owners.
     */
    ...
    /**
     * Returns the current time value under the mouse cursor (in timeline units),
     * computed from the latest mouse move event.
     *
     * @return current time under the cursor
     */
    ...
    /**
     * Returns the combo box that holds all interactive time markers created by
     * clicking while in EXECUTION mode. Selecting an item recenters the viewport
     * around that marker.
     *
     * @return the marker list combo box
     */
    ...
    /**
     * Updates the attributes panel with details from the currently selected
     * {@link MouseTimeLine} (if any). No-op if the list is empty.
     */
    ...
    /**
     * Returns the color palette used for resource legend and mini-bars.
     *
     * @return array of resource colors
     */
     ...
     * color, useful for drawing readable text on filled rectangles.
     *
